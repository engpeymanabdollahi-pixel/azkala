...
class ComprehensiveProductTemplateSeeder extends Seeder
{
    public function run(): void
    {
        ...
        // دریافت دسته‌بندی‌ها با نام، نه با اسلاگ.
        //
        // CategorySeeder اسلاگ را با Str::slug() از نام فارسی می‌سازد، که ارقام
        // و کلمات را حرف‌به‌حرف آوانویسی می‌کند نه ترجمه — «قاب و کاور» به
        // «kab-o-kaor» تبدیل می‌شود، نه به چیزی شبیه «phone-cases». اسلاگ‌های
        // انگلیسیِ قبلی اینجا با هیچ رکورد واقعی مطابقت نداشتند، پس این سیدر
        // همیشه در همان خط بعدی با خطای «ابتدا CategorySeeder را اجرا کنید»
        // متوقف می‌شد — حتی وقتی CategorySeeder واقعاً اجرا شده بود.
        $caseCategory = Category::where('name', 'قاب و کاور')->first();
        $glassCategory = Category::where('name', 'گلس و محافظ صفحه')->first();
        $chargerCategory = Category::where('name', 'شارژر و کابل')->first();
        $audioCategory = Category::where('name', 'هندزفری و هدفون')->first();
        $powerbankCategory = Category::where('name', 'پاوربانک')->first();
        $watchCategory = Category::where('name', 'ساعت هوشمند')->first();
        $holderCategory = Category::where('name', 'هولدر و پایه')->first();
        ...
        // BrandSeeder این دو را همیشه می‌سازد، ولی اگر کسی این سیدر را بدون آن
        // اجرا کند، خطای واضح بهتر از کرش روی $samsung->id چند خط پایین‌تر است.
        if (! $samsung || ! $apple) {
