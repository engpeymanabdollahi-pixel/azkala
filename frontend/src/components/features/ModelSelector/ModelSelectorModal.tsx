...
import { deviceService, DeviceBrand, DeviceSeries, DeviceModel } from '@/services/api/device.service';
...
export function ModelSelectorModal() {
  ...
  const [tempBrand, setTempBrand] = useState<DeviceBrand | null>(null);
  const [tempSeries, setTempSeries] = useState<DeviceSeries | null>(null);
  ...
  useEffect(() => {
    if (isModalOpen) {
      ...
      // ریست کردن state بر اساس انتخاب‌های قبلی
      if (selectedSeries && selectedBrand) {
        setTempBrand({
          id: selectedBrand.id,
          name: selectedBrand.name,
          slug: selectedBrand.slug,
          series_count: selectedBrand.series_count || 0,
        });
        setTempSeries({
          ...
          models_count: selectedSeries.models_count || 0,
        });
        setStep('model');
        loadModels(selectedSeries.id);
  ...
  const loadSeries = async (brandId: number) => {
    ...
    setError(null);
    try {
      const response = await deviceService.getSeries(brandId);
      if (response.success) {
        setSeries(response.data);
      }
    } catch (err) {
      setError('خطا در بارگذاری سری‌ها');
  ...
  const loadModels = async (seriesId: number) => {
    setIsLoadingModels(true);
    setError(null);
    try {
      const response = await deviceService.getModels(seriesId);
      if (response.success) {
  ...
  /**
   * ✅ کلیک روی برند → مستقیم برو به صفحه سری‌ها
   */
  const handleSelectBrand = useCallback(async (brand: DeviceBrand) => {
    setTempBrand(brand);
    setSearchTerm('');
    setStep('series');
    await loadSeries(brand.id);
  }, []);

  /**
   * ✅ کلیک روی سری → مستقیم برو به صفحه مدل‌ها
   */
  const handleSelectSeries = useCallback(async (seriesItem: DeviceSeries) => {
    setTempSeries(seriesItem);
    setSearchTerm('');
    setStep('model');
    await loadModels(seriesItem.id);
  }, []);

  /**
   * ✅ کلیک روی مدل → انتخاب و بسته شدن modal
   */
  const handleSelectModel = useCallback((model: DeviceModel) => {
    if (!tempBrand || !tempSeries) return;

    // تبدیل DeviceBrand به Brand type
    const brandForStore: Brand = {
      ...
      logo: null,
      is_active: true,
      series_count: tempBrand.series_count,
      models_count: 0,
      created_at: '',
      updated_at: '',
    };

    // تبدیل DeviceSeries به PhoneSeries type
    const seriesForStore: PhoneSeries = {
      id: tempSeries.id,
      brand_id: tempBrand.id,
      name: tempSeries.name,
      slug: tempSeries.slug,
      image: tempSeries.image,
      models_count: tempSeries.models_count,
      brand: brandForStore,
      created_at: '',
      updated_at: '',
    };

    // تبدیل DeviceModel به PhoneModel type
  ...
  const isLoading = isLoadingBrands || isLoadingSeries || isLoadingModels;

  // ==================== Render ====================

  return (
    <Modal isOpen={isModalOpen} onClose={closeModal} size="xl" showCloseButton={false}>
      {/* ==================== Header با دکمه‌های واضح ==================== */}
      <div className="flex items-center justify-between p-5 border-b border-gray-100 bg-gradient-to-r from-primary-50 to-white">
        <div className="flex items-center gap-3 flex-1 min-w-0">
          ...
          {step !== 'brand' ? (
            <button
              onClick={handleBack}
              className="w-11 h-11 bg-gradient-to-br from-primary-500 to-primary-600 hover:from-primary-600 hover:to-primary-700 rounded-xl transition-all hover:scale-110 text-white flex-shrink-0 shadow-lg shadow-primary-500/30 hover:shadow-xl flex items-center justify-center group"
              aria-label="برگشت به مرحله قبل"
              title="برگشت به مرحله قبل"
            >
              <ChevronRight className="w-5 h-5 group-hover:-translate-x-0.5 transition-transform" />
            </button>
          ) : (
            // آیکون گوشی در مرحله اول (به جای دکمه برگشت)
            ...
            <div className="flex items-center gap-2 text-xs text-gray-500 mb-1 flex-wrap">
              <button
                ...
                className={cn(
                  ...
                  step === 'brand' && 'text-primary-600 font-bold'
                )}
              ...
              {(step === 'series' || step === 'model') && (
                <>
                  ...
                  <span className={cn(
                    ...
                    step === 'series' ? 'text-primary-600 font-bold' : 'text-gray-700'
                  )}>
                    {tempBrand?.name}
                  </span>
                </>
        ...
        {/* ✅ دکمه ضربدر خروج واضح‌تر */}
        <button
          onClick={closeModal}
          className="w-11 h-11 bg-white hover:bg-error-50 border-2 border-gray-200 hover:border-error-300 rounded-xl transition-all hover:rotate-90 text-gray-500 hover:text-error-500 flex-shrink-0 flex items-center justify-center group shadow-sm hover:shadow-md"
          aria-label="بستن"
          title="بستن پنجره"
        >
        ...
        {(['brand', 'series', 'model'] as Step[]).map((s, idx) => {
          ...
          return (
            <div key={s} className="flex items-center gap-2 flex-1">
              ...
              {idx < 2 && (
                <div className={cn(
                  'flex-1 h-0.5 rounded-full mx-2 transition-all',
                  isCompleted ? 'bg-gradient-to-r from-success-500 to-success-600' : 'bg-gray-200'
                )} />
              )}
            </div>
          );
          ...
          <div className="mb-4 bg-error-50 border border-error-200 rounded-2xl p-4">
            ...
            <button
              onClick={() => {
                ...
                else if (step === 'series' && tempBrand) loadSeries(tempBrand.id);
                else if (step === 'model' && tempSeries) loadModels(tempSeries.id);
              }}
              className="text-error-600 text-xs font-bold hover:underline"
            >
              تلاش مجدد
            </button>
          </div>
                    ...
                    <div className="flex flex-wrap gap-2">
                      {popularBrands.map((brand) => (
                        <button
                          ...
                          className="flex items-center gap-2 px-3 py-2 rounded-xl border-2 border-gray-200 bg-white hover:border-primary-300 hover:bg-primary-50/50 transition-all text-sm font-semibold"
                        >
                  ...
                  {filteredBrands.length > 0 ? (
                    filteredBrands.map((brand, index) => (
                      <button
                        key={brand.id}
                        onClick={() => handleSelectBrand(brand)}
                        className="group p-4 bg-white border-2 border-gray-200 rounded-2xl transition-all duration-300 text-center relative overflow-hidden hover:border-primary-500 hover:shadow-xl hover:-translate-y-1"
                      ...
                      >
                        ...
                        <span className="font-black text-sm block text-gray-800 group-hover:text-primary-600 transition-colors truncate">
                          {brand.name}
                        </span>
                        <span className="text-xs text-gray-400 mt-1 block">
                          {brand.series_count} سری
                        </span>
                    ...
                    <div className="col-span-full text-center py-12">
                      ...
                      <p className="text-sm text-gray-500">
                        برندی با نام "{searchTerm}" در لیست ما وجود ندارد
                      </p>
                    </div>
                  )}
                </div>
                ...
                {filteredSeries.length > 0 ? (
                  filteredSeries.map((s, index) => (
                    <button
                      ...
                      className="flex items-center gap-4 p-4 bg-white border-2 border-gray-200 rounded-2xl transition-all duration-300 group relative overflow-hidden hover:border-primary-500 hover:shadow-xl hover:-translate-y-1"
                      style={{ animationDelay: `${index * 30}ms` }}
                    >
                      ...
                      <div className="text-right flex-1 min-w-0">
                        <span className="font-black text-gray-900 block group-hover:text-primary-600 transition-colors truncate">
                          {s.name}
                        </span>
                  ...
                  <div className="col-span-full text-center py-12">
                    ...
                    <p className="text-sm text-gray-500">
                      سری‌ای با نام "{searchTerm}" برای برند {tempBrand?.name} یافت نشد
                    </p>
                  </div>
                )}
              </div>
                ...
                {filteredModels.length > 0 ? (
                  filteredModels.map((model, index) => (
                    <button
                      ...
                      className="flex items-center gap-4 p-4 bg-white border-2 border-gray-200 rounded-2xl transition-all duration-300 hover:border-success-500 hover:shadow-xl hover:-translate-y-1 group relative overflow-hidden"
                      style={{ animationDelay: `${index * 30}ms` }}
                    >
                      ...
                      <div className="text-right flex-1 min-w-0">
                        <span className="font-black text-gray-900 block text-sm group-hover:text-success-600 transition-colors truncate">
                          {model.name}
                        </span>
                        {model.release_year && (
                          <span className="text-xs text-gray-400 flex items-center gap-1 mt-0.5">
                  ...
                  <div className="col-span-full text-center py-12">
                    ...
                    <p className="text-sm text-gray-500">
                      مدلی با نام "{searchTerm}" برای سری {tempSeries?.name} یافت نشد
                    </p>
                  </div>
                )}
              </div>
      ...
      {/* ✅ Footer حذف شد - فقط یک راهنمای کوچک */}
      {step === 'brand' && brands.length > 0 && !searchTerm && (
