 ...
 * کارت پایه‌ی ازکالا.
 *
 * ✅ این کامپوننت قبلاً با یک primitive خام و بدون‌استایل به سبک shadcn
 * جایگزین شده بود (بدون variant/interactive/entranceDelay) در حالی که
 * نسخه‌ی اصلی و کامل‌تر به‌عنوان کد مرده در Card.old.tsx باقی مانده بود.
 * تنها سه مصرف‌کننده‌ی واقعیِ این فایل (ChatStatsWidget، SentimentWidget،
 * RecentChatActivityWidget) همگی از variant="elevated" استفاده می‌کنند که
 * در نسخه‌ی خام اصلاً پاس داده نمی‌شد — یعنی سایه‌ی «elevated» هیچ‌وقت واقعاً
 * رندر نمی‌شد. صادرات CardHeader/CardTitle/CardDescription/CardContent/
 * CardFooter نسخه‌ی قبلی در کل پروژه هیچ مصرف‌کننده‌ای نداشتند (فقط در همان
 * فایل تعریف شده بودند)، پس بازگرداندن این نسخه هیچ مصرف‌کننده‌ای را نمی‌شکند.
 *
