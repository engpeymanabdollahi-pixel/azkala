...
import {
  ...
  Users, Calendar, TrendingUp, Shield, UserX,
} from 'lucide-react';
...
export function AdminBlocksPage() {
  ...
  const handleBlockByAdmin = async () => {
    ...
    try {
      ...
      toast.error(axiosError.response?.data?.message || 'خطا در بلاک کردن');
    } finally {
      setIsBlocking(false);
    }
  };
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-black text-gray-900 dark:text-gray-100">مدیریت بلاک کاربران</h1>
            <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">مشاهده و مدیریت کاربران بلاک شده</p>
          </div>
        </div>
        <Button
      ...
      {stats && (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <div className="bg-white dark:bg-gray-800 rounded-xl p-4 border border-gray-100 dark:border-gray-700 shadow-sm">
            ...
            <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">کل بلاک‌ها</p>
          </div>
          ...
          <div className="bg-white dark:bg-gray-800 rounded-xl p-4 border border-gray-100 dark:border-gray-700 shadow-sm">
            ...
            <p className="text-2xl font-black text-orange-600 dark:text-orange-400">{stats.today.toLocaleString('fa-IR')}</p>
            <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">بلاک امروز</p>
          </div>
          ...
          <div className="bg-white dark:bg-gray-800 rounded-xl p-4 border border-gray-100 dark:border-gray-700 shadow-sm">
            ...
            <p className="text-2xl font-black text-blue-600 dark:text-blue-400">{stats.week.toLocaleString('fa-IR')}</p>
            <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">بلاک هفته</p>
          </div>
          ...
          <div className="bg-white dark:bg-gray-800 rounded-xl p-4 border border-gray-100 dark:border-gray-700 shadow-sm">
            ...
            <p className="text-2xl font-black text-purple-600 dark:text-purple-400">{stats.month.toLocaleString('fa-IR')}</p>
            <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">بلاک ماه</p>
          </div>
        </div>
      )}
          ...
          <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-100 dark:border-gray-700 shadow-sm p-4">
            ...
            <div className="space-y-2">
              {stats.most_blocked_users.map((user, i) => (
                <div key={user.id} className="flex items-center justify-between p-2 bg-gray-50 dark:bg-gray-900/60 rounded-lg">
                  <div className="flex items-center gap-2">
                    <span className="w-6 h-6 bg-red-100 dark:bg-red-900/40 text-red-700 dark:text-red-400 rounded-full flex items-center justify-center text-xs font-bold">
                      {i + 1}
                    </span>
          ...
          <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-100 dark:border-gray-700 shadow-sm p-4">
            ...
            <div className="space-y-2">
              {stats.most_blockers.map((user, i) => (
                <div key={user.id} className="flex items-center justify-between p-2 bg-gray-50 dark:bg-gray-900/60 rounded-lg">
                  <div className="flex items-center gap-2">
                    <span className="w-6 h-6 bg-blue-100 dark:bg-blue-900/40 text-blue-700 dark:text-blue-400 rounded-full flex items-center justify-center text-xs font-bold">
                      {i + 1}
                    </span>
                    <span className="text-sm font-bold text-gray-900 dark:text-gray-100">{user.name}</span>
                  </div>
      ...
      <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-100 dark:border-gray-700 shadow-sm overflow-hidden">
        {isLoading ? (
          ...
          <div className="divide-y divide-gray-100 dark:divide-gray-700">
            {blocks.map((block) => (
              <div key={block.id} className="p-4 hover:bg-gray-50 dark:hover:bg-gray-700/30 transition-colors">
                <div className="flex items-start justify-between gap-3">
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-4 mb-2">
                      <div className="flex items-center gap-2">
                        ...
                        <div>
                          <p className="text-xs text-gray-500 dark:text-gray-400">بلاک‌کننده</p>
                          <p className="text-sm font-bold text-gray-900 dark:text-gray-100">{block.user?.name || 'ناشناس'}</p>
                        </div>
                      </div>
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <div>
                          <p className="text-xs text-gray-500 dark:text-gray-400">بلاک‌شده</p>
                          <p className="text-sm font-bold text-red-600 dark:text-red-400">{block.blockedUser?.name || 'ناشناس'}</p>
                        </div>
                      </div>
                    </div>
                    ...
                    {block.reason && (
                      <div className="bg-gray-50 dark:bg-gray-900/60 rounded-lg p-2 text-xs">
                        <span className="font-bold text-gray-700 dark:text-gray-300">دلیل:</span>
                        <span className="text-gray-600 dark:text-gray-400 ml-1">{block.reason}</span>
                      </div>
                    )}
                  </div>
                  ...
                  <Button
                    ...
                    className="gap-1 flex-shrink-0 bg-green-50 dark:bg-green-900/20 border-green-200 dark:border-green-800 text-green-700 dark:text-green-400 hover:bg-green-100 dark:hover:bg-green-900/40"
                  >
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white dark:bg-gray-800 rounded-2xl w-full max-w-md shadow-2xl">
            ...
            <div className="p-4 space-y-4">
              <div>
                <label className="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-1">ID کاربر</label>
                <input
                  type="number"
                  value={blockUserId}
                  onChange={(e) => setBlockUserId(e.target.value)}
                  placeholder="شناسه عددی کاربر..."
                  className="w-full px-3 py-2 border border-gray-200 dark:border-gray-700 rounded-lg text-sm bg-white dark:bg-gray-900 text-gray-900 dark:text-gray-100 placeholder:text-gray-400 dark:placeholder:text-gray-500 focus:outline-none focus:border-red-500"
                />
              </div>
              <div>
                <label className="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-1">دلیل بلاک</label>
                <textarea
                  value={blockReason}
                  onChange={(e) => setBlockReason(e.target.value)}
                  placeholder="دلیل بلاک کردن..."
                  rows={3}
                  className="w-full px-3 py-2 border border-gray-200 dark:border-gray-700 rounded-lg text-sm bg-white dark:bg-gray-900 text-gray-900 dark:text-gray-100 placeholder:text-gray-400 dark:placeholder:text-gray-500 focus:outline-none focus:border-red-500 resize-none"
                  maxLength={500}
                />
              </div>
