...
export function AdminCouponsPage() {
  ...
  // ✅ couponService.getAllCoupons() از قبل پارامتر page را به بکند
  // می‌فرستاد (که خودش صفحه‌بندی واقعی برمی‌گرداند)، اما هیچ‌جا در این
  // صفحه state/UI صفحه‌بندی وجود نداشت — یعنی برای هر فروشگاهی با بیش از
  // ۲۰ کد تخفیف، ادمین هیچ‌وقت نمی‌توانست فراتر از صفحه‌ی اول را ببیند.
  const [page, setPage] = useState(1);

  // جستجو با تاخیر برای جلوگیری از درخواست به ازای هر حرف
  useEffect(() => {
  ...
  const { data: couponsData, isLoading } = useQuery({
    queryKey: ['admin-coupons', page, debouncedSearch, filterStatus, filterType],
  ...
  const coupons: Coupon[] = couponsData?.data?.coupons || [];
  const pagination = couponsData?.data?.pagination;
  const stats = couponsData?.data?.stats;
  ...
  const handleSubmit = () => {
    ...
    const submitData: Omit<CouponFormData, 'start_date' | 'end_date'> & {
    ...
    return (
      <div className="min-h-screen bg-gray-50 dark:bg-gray-900 p-6">
        <div className="max-w-7xl mx-auto">
          <div className="h-10 bg-white dark:bg-gray-800 rounded-xl mb-6 animate-pulse" />
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900 p-4 md:p-6">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-2 md:grid-cols-5 gap-3 mb-6">
          <StatCard icon={Package} label="کل کدها" value={stats?.total || 0} color="primary" />
          <StatCard icon={CheckCircle} label="فعال" value={stats?.active || 0} color="success" />
          <StatCard icon={Percent} label="درصدی" value={stats?.percentage || 0} color="accent" />
          <StatCard icon={DollarSign} label="مبلغ ثابت" value={stats?.fixed || 0} color="warning" />
          <StatCard icon={Users} label="کل استفاده" value={stats?.total_usage || 0} color="primary" />
        </div>
          ...
          <div className="flex flex-col md:flex-row gap-3">
            ...
            <div className="flex gap-2">
              <select
                ...
                className="px-3 py-2 border border-gray-200 dark:border-gray-700 rounded-lg text-sm focus:outline-none focus:border-primary-500 bg-white dark:bg-gray-900 text-gray-900 dark:text-gray-100"
              >
              ...
              <table className="w-full">
                <thead>
                  <tr className="bg-gray-50 dark:bg-gray-900/60 border-b border-gray-100 dark:border-gray-700">
                    <th className="text-right px-4 py-3 text-xs font-bold text-gray-600 dark:text-gray-300">کد</th>
                    <th className="text-right px-4 py-3 text-xs font-bold text-gray-600 dark:text-gray-300">نوع</th>
                    <th className="text-right px-4 py-3 text-xs font-bold text-gray-600 dark:text-gray-300">مقدار</th>
                    <th className="text-right px-4 py-3 text-xs font-bold text-gray-600 dark:text-gray-300">حداقل سفارش</th>
                    <th className="text-right px-4 py-3 text-xs font-bold text-gray-600 dark:text-gray-300">استفاده</th>
                    <th className="text-right px-4 py-3 text-xs font-bold text-gray-600 dark:text-gray-300">اعتبار</th>
                    <th className="text-right px-4 py-3 text-xs font-bold text-gray-600 dark:text-gray-300">وضعیت</th>
                    <th className="text-center px-4 py-3 text-xs font-bold text-gray-600 dark:text-gray-300">عملیات</th>
                  </tr>
                </thead>
                <tbody>
                  {coupons.map((coupon) => (
                    <tr key={coupon.id} className="border-b border-gray-50 dark:border-gray-700/50 hover:bg-gray-50/50 dark:hover:bg-gray-700/30 transition-colors">
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-2">
                          <code className="bg-primary-50 dark:bg-primary-900/30 text-primary-700 dark:text-primary-400 px-2 py-1 rounded-md text-xs font-bold">
                            {coupon.code}
                          </code>
                          <button
                        ...
                        {coupon.description && (
                          <p className="text-[10px] text-gray-500 dark:text-gray-400 mt-1 line-clamp-1">{coupon.description}</p>
                        )}
                      </td>
                      <td className="px-4 py-3">
                      ...
                      <td className="px-4 py-3">
                        ...
                        {coupon.max_discount && coupon.type === 'percentage' && (
                          <p className="text-[10px] text-gray-500 dark:text-gray-400">حداکثر {formatPrice(coupon.max_discount)}</p>
                        )}
                      </td>
                      <td className="px-4 py-3">
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-1.5">
                          ...
                          <span className="text-sm font-semibold text-gray-700 dark:text-gray-300">{coupon.used_count || 0}</span>
                          {coupon.usage_limit && <span className="text-[10px] text-gray-500 dark:text-gray-400">/ {coupon.usage_limit}</span>}
                        </div>
                        <p className="text-[10px] text-gray-500 dark:text-gray-400">هر کاربر: {coupon.usage_limit_per_user}x</p>
                      </td>
                      <td className="px-4 py-3">
                        <div className="text-xs text-gray-600 dark:text-gray-300">
                          ...
                          {!coupon.start_date && !coupon.end_date && <span className="text-gray-400 dark:text-gray-500">نامحدود</span>}
                        </div>
                      </td>
                      <td className="px-4 py-3">
                        <button
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center justify-center gap-1">
                          <button onClick={() => handleOpenEdit(coupon)} className="p-1.5 hover:bg-primary-50 dark:hover:bg-primary-900/30 rounded-lg text-gray-500 dark:text-gray-400 hover:text-primary-600 dark:hover:text-primary-400 transition-colors" title="ویرایش">
                            <Edit2 className="w-4 h-4" />
                          </button>
                          <button onClick={() => handleDelete(coupon.id)} className="p-1.5 hover:bg-error-50 dark:hover:bg-error-900/30 rounded-lg text-gray-500 dark:text-gray-400 hover:text-error-600 dark:hover:text-error-400 transition-colors" title="حذف">
        ...
        {coupons.length > 0 && (
          <div className="mt-3 flex flex-col md:flex-row items-center justify-between gap-3">
            <p className="text-xs text-gray-500 dark:text-gray-400">
              نمایش {coupons.length} کد از {pagination?.total ?? coupons.length} کد
            </p>
            {/* ✅ قبلاً هیچ کنترل صفحه‌بندی‌ای وجود نداشت؛ برای فروشگاهی با بیش
                از ۲۰ کد تخفیف، ادمین هرگز نمی‌توانست به صفحات بعدی برسد. */}
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-hidden flex flex-col">
            <div className="flex items-center justify-between p-5 border-b border-gray-100 dark:border-gray-700 bg-gradient-to-l from-primary-50/50 dark:from-primary-900/20 to-white dark:to-gray-800">
              ...
              <button onClick={handleCloseForm} className="p-1.5 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-lg transition-colors">
            ...
            <div className="flex-1 overflow-y-auto p-5 space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <select
                    ...
                    className="w-full px-3 py-2 border border-gray-200 dark:border-gray-700 rounded-lg text-sm focus:outline-none focus:border-primary-500 bg-white dark:bg-gray-900 text-gray-900 dark:text-gray-100"
                  >
                    <option value="percentage">درصدی (٪)</option>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                {formData.type === 'percentage' && (
                  <div>
                    <label className="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-1.5">حداکثر مبلغ تخفیف</label>
                    <input
                      type="number"
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-1.5">حداقل مبلغ سفارش</label>
                  <input
                    type="number"
                ...
                <div>
                  <label className="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-1.5">محدودیت هر کاربر</label>
                  <input
                    type="number"
              ...
              <div>
                <label className="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-1.5">محدودیت کلی استفاده</label>
                <input
                  type="number"
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-1.5">تاریخ شروع</label>
                  <input
                    type="date"
                    value={formData.start_date}
                    onChange={(e) => setFormData(prev => ({ ...prev, start_date: e.target.value }))}
                    className="w-full px-3 py-2 border border-gray-200 dark:border-gray-700 rounded-lg text-sm bg-white dark:bg-gray-900 text-gray-900 dark:text-gray-100 focus:outline-none focus:border-primary-500"
                  />
                </div>
                <div>
                  <label className="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-1.5">تاریخ پایان</label>
                  <input
                    type="date"
              ...
              <div>
                <label className="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-1.5">توضیحات</label>
                <textarea
              ...
              <div className="flex items-center justify-between p-3 bg-gray-50 dark:bg-gray-900/60 rounded-lg border border-gray-100 dark:border-gray-700">
                <div>
                  <p className="text-sm font-bold text-gray-900 dark:text-gray-100">وضعیت فعال</p>
                  <p className="text-[10px] text-gray-500 dark:text-gray-400">در صورت غیرفعال بودن کد قابل استفاده نخواهد بود</p>
                </div>
                <button
                  type="button"
...
function StatCard({ icon: Icon, label, value, color }: {
  ...
  return (
    <div className="bg-white dark:bg-gray-800 rounded-xl p-4 border border-gray-100 dark:border-gray-700 shadow-sm">
      ...
      <p className="text-2xl font-black text-gray-900 dark:text-gray-100">{value.toLocaleString('fa-IR')}</p>
    </div>
  );
}
