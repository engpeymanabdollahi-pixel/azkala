...
import {
  ShoppingCart, Search, Filter, Eye, Edit2, Truck, CheckCircle,
...
const getStatusInfo = (status: string) => {
  const map: Record<string, { label: string; color: string; icon: any; bg: string }> = {
    ...
    processing: { label: 'در حال پردازش', color: 'primary', icon: Package, bg: 'bg-primary-50 text-primary-700 border-primary-200' },
    shipped: { label: 'ارسال شده', color: 'accent', icon: Truck, bg: 'bg-accent-50 text-accent-700 border-accent-200' },
    delivered: { label: 'تحویل شده', color: 'success', icon: CheckCircle, bg: 'bg-success-50 text-success-700 border-success-200' },
    cancelled: { label: 'لغو شده', color: 'error', icon: X, bg: 'bg-error-50 text-error-700 border-error-200' },
    returned: { label: 'مرجوعی', color: 'gray', icon: AlertCircle, bg: 'bg-gray-50 text-gray-700 border-gray-200' },
  };
        ...
        {sellers.length > 0 && (
          <div className="flex items-center gap-2 pt-3 border-t border-gray-100">
            <span className="text-xs font-bold text-gray-700 flex items-center gap-1.5">
              <Store className="w-3.5 h-3.5 text-accent-600" />
              فروشنده:
            </span>
            <select
              ...
              className="px-3 py-2 border border-gray-200 rounded-lg text-xs font-semibold focus:outline-none focus:border-primary-500 bg-white min-w-[200px]"
            >
    ...
    return (
      <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
        <div className="bg-white rounded-2xl shadow-2xl max-w-lg w-full p-8 text-center">
          ...
          <p className="text-gray-700 font-bold">اطلاعات سفارش در دسترس نیست</p>
  ...
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl shadow-2xl max-w-3xl w-full max-h-[90vh] overflow-hidden flex flex-col">
        {/* Header */}
        <div className="flex items-center justify-between p-5 border-b border-gray-100 bg-gradient-to-l from-accent-50/50 to-white">
          <div>
            ...
            <p className="text-xs text-gray-500 mt-1">
              شماره سفارش: <code className="font-bold text-primary-700">{order.order_number}</code>
            </p>
          </div>
            ...
            <div className="bg-accent-50 rounded-xl p-4 border border-accent-100">
              ...
              <div className="flex flex-wrap gap-2">
                {order.sellers.map((seller) => (
                  <div
                    key={seller.id}
                    className="inline-flex items-center gap-1.5 px-2.5 py-1.5 bg-white border border-accent-200 rounded-lg text-xs font-bold text-accent-700"
                  >
          ...
          <div>
            ...
            <div className="space-y-2">
              {itemsData.map((item) => (
                <div
                  key={item.id}
                  className="flex items-center gap-3 p-3 bg-white border border-gray-100 rounded-xl hover:border-accent-200 transition-all"
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-3 mt-1 text-xs text-gray-500">
                      <span>تعداد: <strong className="text-gray-900">{item.quantity}</strong></span>
                      <span>قیمت واحد: <strong className="text-gray-900">{formatPrice(item.price)}</strong></span>
                    </div>
                  </div>
                  <div className="text-left">
                    <p className="text-sm font-black text-accent-700">{formatPrice(item.total)}</p>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div className="bg-gradient-to-br from-gray-50 to-white rounded-xl p-4 border border-gray-100">
            ...
            <div className="space-y-2">
              <div className="flex justify-between text-sm">
                ...
                <span className="font-bold text-gray-900">{formatPrice(orderData.subtotal)}</span>
              </div>
              {orderData.discount > 0 && (
                <div className="flex justify-between text-sm">
                  <span className="text-gray-600">تخفیف:</span>
                  <span className="font-bold text-success-600">- {formatPrice(orderData.discount)}</span>
                </div>
              )}
              <div className="flex justify-between text-sm">
                <span className="text-gray-600">هزینه ارسال:</span>
                <span className="font-bold text-gray-900">{formatPrice(orderData.shipping)}</span>
              </div>
              {orderData.tax > 0 && (
                <div className="flex justify-between text-sm">
                  <span className="text-gray-600">مالیات:</span>
                  <span className="font-bold text-gray-900">{formatPrice(orderData.tax)}</span>
                </div>
              )}
              <div className="flex justify-between text-base pt-2 border-t border-gray-200">
                <span className="font-black text-gray-900">مبلغ نهایی:</span>
                <span className="font-black text-primary-700">{formatPrice(orderData.total)}</span>
              </div>
              {orderData.coupon_code && (
                <div className="flex items-center gap-1.5 pt-2 text-xs">
                  ...
                  <code className="font-bold text-accent-700 bg-accent-50 px-1.5 py-0.5 rounded">
                    {orderData.coupon_code}
                  </code>
                </div>
              )}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              {orderData.tracking_number && (
                <div className="bg-accent-50 rounded-xl p-3 border border-accent-100">
                  ...
                  <code className="text-sm font-bold text-accent-700">
                    {orderData.tracking_number}
                  </code>
                </div>
              )}
        ...
        <div className="p-5 space-y-4">
          <div className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
            <div>
              ...
              <code className="text-sm font-bold text-primary-700">{order.order_number}</code>
            </div>
            <div className="text-left">
          ...
          <div>
            <label className="block text-sm font-bold text-gray-700 mb-2">وضعیت سفارش</label>
          ...
          {(status === 'shipped' || status === 'delivered') && (
            <div>
              <label className="block text-sm font-bold text-gray-700 mb-1.5">کد پیگیری</label>
              <input
                type="text"
          ...
          <div>
            <label className="block text-sm font-bold text-gray-700 mb-1.5">یادداشت</label>
            <textarea
              value={notes}
          ...
          <div>
            <label className="block text-sm font-bold text-gray-700 mb-2">وضعیت پرداخت</label>
            <div className="grid grid-cols-2 gap-2">
              {[
                { value: 'pending', label: 'پرداخت نشده', color: 'warning' },
                { value: 'paid', label: 'پرداخت شده', color: 'success' },
                { value: 'failed', label: 'ناموفق', color: 'error' },
                { value: 'refunded', label: 'مسترد شده', color: 'gray' },
              ].map((p) => (
                <button
                  key={p.value}
