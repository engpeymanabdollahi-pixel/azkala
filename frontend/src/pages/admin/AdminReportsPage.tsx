    ...
    <div className="space-y-5">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">تحلیل جامع عملکرد پلتفرم ازکالا</p>
        </div>
      ...
      <div className="card-enhanced">
        <div className="flex items-center justify-between mb-4 flex-wrap gap-2">
          <div>
            ...
            <p className="text-xs text-gray-500 mt-1">روند فروش در {period} روز اخیر</p>
          </div>
          {summary && (
            <div className="flex items-center gap-4 text-xs">
              <div className="text-center">
                <p className="text-gray-500">کل فروش</p>
            ...
            {topProducts.map((product, index) => (
              <div key={product.id} className="flex items-center gap-3 p-2 rounded-lg hover:bg-gray-50 transition-colors">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-[10px] text-gray-500">{product.total_sold.toLocaleString('fa-IR')} فروش</p>
                </div>
              ...
              <div className="grid grid-cols-2 gap-2 mt-4">
                {topCategories.slice(0, 6).map((cat, index) => (
                  <div key={cat.id} className="flex items-center gap-2 text-xs">
                    ...
                    />
                    <span className="text-gray-700 line-clamp-1">{cat.name}</span>
                  </div>
                ))}
              </div>
            </>
          ) : (
            <p className="text-center text-sm text-gray-500 py-8">داده‌ای وجود ندارد</p>
          )}
        </div>
      </div>
            ...
            return (
              <div key={item.status} className="space-y-1">
                <div className="flex items-center justify-between text-xs">
                  <span className="font-bold text-gray-700">{getStatusLabel(item.status)}</span>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="bg-gradient-to-br from-primary-50 to-white p-4 rounded-xl border border-primary-100">
            <div className="flex items-center justify-between mb-2">
              <span className="text-sm font-bold text-gray-700">مکالمات</span>
          ...
          <div className="bg-gradient-to-br from-accent-50 to-white p-4 rounded-xl border border-accent-100">
            <div className="flex items-center justify-between mb-2">
              <span className="text-sm font-bold text-gray-700">پیام‌ها</span>
      ...
      <div className="card-enhanced p-5">
          ...
          {(analysis.daily_stats || []).map((day, i) => {
            ...
            return (
              <div key={i} className="flex items-center gap-3 text-sm">
                <span className="w-16 text-xs text-gray-500 font-bold">{day.label}</span>
                <div className="flex-1 space-y-1">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="w-12 text-xs text-gray-600 text-right">{day.conversations}</span>
                  </div>
                  <div className="flex items-center gap-2">
                    <div className="flex-1 flex h-2 rounded-full overflow-hidden bg-gray-100">
                      <div
                    ...
                    <span className="w-12 text-xs text-gray-600 text-right">{day.messages}</span>
                  </div>
                </div>
              </div>
            );
        ...
        <div className="flex items-center justify-center gap-6 mt-4 pt-3 border-t border-gray-100">
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs text-gray-600">مکالمات</span>
          </div>
          <div className="flex items-center gap-2">
            <div className="w-3 h-3 rounded-full bg-accent-500" />
            <span className="text-xs text-gray-600">پیام‌ها</span>
          </div>
        </div>
      </div>
          ...
          {(analysis.hourly_stats || []).map((hour, i) => {
            ...
            return (
              <div key={i} className="text-center">
                ...
                />
                <p className="text-[10px] text-gray-500 mt-1">{hour.label}</p>
                <p className="text-[10px] font-bold text-gray-700">{hour.count}</p>
              </div>
            );
          })}
        </div>
                  ...
                  <tr key={seller.id}>
                    ...
                    <td>
                      <div className="flex items-center gap-2">
                        ...
                        <div>
                          <p className="text-sm font-bold text-gray-900">{seller.shop_name || seller.name}</p>
    ...
    <div className="space-y-5">
      <div className="card-enhanced p-5">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          {metrics.map((metric) => (
            <div key={metric.label} className="bg-gradient-to-br from-gray-50 to-white p-4 rounded-xl border border-gray-100">
              ...
              <div className="space-y-2">
                <div>
                  <p className="text-[10px] text-gray-500">دوره فعلی</p>
                  <p className="text-xl font-black text-gray-900">
                    {metric.isMoney ? formatPrice(metric.current) : Number(metric.current).toLocaleString('fa-IR')}
                  </p>
                </div>
                <div>
                  <p className="text-[10px] text-gray-500">دوره قبل</p>
          ...
          {(analytics.by_brand || []).map((item, index) => {
            ...
            return (
              <div key={item.device_brand} className="flex items-center gap-3">
                <div className="w-6 text-xs font-black text-gray-500">{index + 1}</div>
                <div className="flex-1">
                  <div className="flex items-center justify-between text-xs mb-1">
                    <span className="font-bold text-gray-700">{item.device_brand}</span>
                    <span className="text-gray-500">
                      {item.count.toLocaleString('fa-IR')} ({percentage.toFixed(1)}%)
                    </span>
                  </div>
          ...
          {(analytics.by_model || []).map((item, index) => {
            ...
            return (
              <div key={item.device_model} className="flex items-center gap-3">
                ...
                <div className="flex-1">
                  <div className="flex items-center justify-between text-xs mb-1">
                    <span className="font-bold text-gray-700">{item.device_model}</span>
                    <span className="text-gray-500">{item.count.toLocaleString('fa-IR')}</span>
                  </div>
                  <div className="progress-enhanced">
                    <div
                      className="progress-enhanced-bar"
      ...
      <div className="card-enhanced p-5">
        ...
        <div className="space-y-2">
          {(analysis.frequently_bought || []).map((item, index) => (
            <div key={item.product_id} className="flex items-center gap-3 p-2 rounded-lg hover:bg-gray-50">
          ...
          {(predictions.predictions || []).map((pred, index) => (
            <div key={pred.date} className="flex items-center gap-3 p-3 rounded-lg bg-gradient-to-l from-accent-50 to-white border border-accent-100">
              ...
              <div className="flex-1">
                ...
                <p className="text-lg font-black text-gray-900">{formatPrice(pred.predicted_revenue || 0)}</p>
              </div>
  ...
  if (isError || !data?.success || !data?.data) {
    return (
      <div className="card-enhanced p-8 text-center">
        ...
        <p className="text-sm font-bold text-gray-700">خطا در دریافت داده‌ها</p>
      ...
      <div className="card-enhanced p-5">
        ...
        {anomalyList.length > 0 ? (
              ...
              return (
                ...
                >
                  ...
                  <div className="grid grid-cols-3 gap-2 text-xs">
                    <div>
                      <p className="text-gray-500">درآمد</p>
                      <p className="font-black text-gray-900">{formatPrice(revenue)}</p>
                    </div>
                    <div>
                      <p className="text-gray-500">سفارشات</p>
                      <p className="font-black text-gray-900">{ordersCount}</p>
                    </div>
                    <div>
                      <p className="text-gray-500">انحراف</p>
          ...
          <div className="empty-state-enhanced">
            ...
            <p className="text-sm font-bold text-gray-700">ناهنجاری شناسایی نشد</p>
            <p className="text-xs text-gray-500 mt-1">همه چیز عادی است!</p>
          </div>
        )}
      </div>
    </div>
...
const PieChart = memo(function PieChart({ data, size = 240 }: { data: TopCategory[]; size?: number }) {
  ...
  if (total === 0) return <p className="text-sm text-gray-500 py-8 text-center">داده‌ای وجود ندارد</p>;
