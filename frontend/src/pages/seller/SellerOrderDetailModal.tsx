...
import {
  Package, User, MapPin, Clock, CheckCircle, Truck,
  Printer, Phone, Calendar,
  Copy, DollarSign, ShoppingBag, Sparkles, Tag,
...
export function SellerOrderDetailModal({ isOpen, onClose, orderId }: SellerOrderDetailModalProps) {
  ...
  const updateStatusMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      // کلید واقعیِ لیست سفارشات 'seller-orders-fresh' است، نه 'seller-orders'
      // (نگاه کنید به SellerOrders.tsx) — این invalidate قبلاً کلید غلطی را
      // می‌زد، پس بعد از تغییر وضعیت از این مودال، لیست پشت سرش هیچ‌وقت
      // خودکار به‌روز نمی‌شد.
  ...
  // Parse shipping_address — با cast آرایه‌ای Order مدل، بک‌اند دیگر رشته‌ی
  // JSON خام برنمی‌گرداند، ولی این چک برای سازگاری با داده‌ی قدیمی‌تر می‌ماند.
  ...
  const timelineSteps = useMemo(() => {
    ...
    return ORDER_TIMELINE_STEPS.map((step) => ({
      ...step,
      time: order.status === step.status ? order.updated_at : (step.status === 'pending' ? order.created_at : null),
      ...
      <Modal isOpen={isOpen} onClose={onClose} size="xl">
        <div className="flex items-center justify-center py-20">
          <div className="text-center">
            ...
            <p className="text-gray-600 dark:text-gray-400 text-sm">در حال بارگذاری...</p>
          </div>
        </div>
      </Modal>
    );
    ...
    <>
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="flex items-center gap-2 mb-0.5">
                <h1 className="text-lg font-black text-gray-900 dark:text-white">{order.order_number}</h1>
            ...
            <div className="lg:col-span-2 space-y-3">
              ...
              <div className="bg-white dark:bg-slate-800 rounded-xl border border-gray-100 dark:border-slate-700 shadow-sm p-3">
                <div className="flex items-center gap-1.5 mb-3">
                  ...
                  <h2 className="text-sm font-black text-gray-900 dark:text-white">وضعیت سفارش</h2>
                </div>

                <div className="relative">
                  <div className="absolute right-[15px] top-0 bottom-0 w-0.5 bg-gray-200 dark:bg-slate-700" />
                    ...
                    {timelineSteps.map((step, index) => {
                      ...
                      return (
                        <div key={index} className="flex items-start gap-3 relative">
                          ...
                          >
                            {step.active ? <Icon className="w-4 h-4" /> : <div className="w-2 h-2 bg-gray-300 dark:bg-slate-600 rounded-full" />}
                          </div>
                          <div className={cn('flex-1 pt-0.5 pb-1.5', step.active ? 'opacity-100' : 'opacity-50')}>
                            <div className="flex items-center gap-1.5 mb-0.5">
                              <p className={cn('font-black text-sm', step.active ? 'text-gray-900 dark:text-white' : 'text-gray-400 dark:text-gray-500')}>
                                {step.label}
                              </p>
                              {isCurrent && (
                            ...
                            <p className={cn('text-xs mb-0.5', step.active ? 'text-gray-600 dark:text-gray-400' : 'text-gray-400 dark:text-gray-500')}>
                              {step.description}
                            </p>
                            {step.time && (
              ...
              <div className="bg-white dark:bg-slate-800 rounded-xl border border-gray-100 dark:border-slate-700 shadow-sm overflow-hidden">
                ...
                <div className="divide-y divide-gray-100 dark:divide-slate-700">
                  {(order.items || []).map((item, index) => (
                    <div key={index} className="p-3 hover:bg-primary-50/30 dark:hover:bg-primary-900/10 transition-colors group">
                      <div className="flex items-start gap-2">
                        ...
                        <div className="flex-1 min-w-0">
                          <h3 className="font-black text-gray-900 dark:text-white text-sm mb-0.5 group-hover:text-primary-600 dark:group-hover:text-primary-400 transition-colors line-clamp-1">
                            {item.product?.name || 'محصول'}
                          </h3>
                          <div className="flex flex-wrap items-center gap-1.5 text-[11px]">
                            <span className="flex items-center gap-0.5 bg-gray-100 dark:bg-slate-700 px-1.5 py-0.5 rounded text-gray-700 dark:text-gray-300">
                              <Package className="w-3 h-3 text-primary-500 dark:text-primary-400" />
                              <span className="font-semibold">{item.quantity}</span> عدد
                            </span>
                            <span className="flex items-center gap-0.5 bg-gray-100 dark:bg-slate-700 px-1.5 py-0.5 rounded text-gray-700 dark:text-gray-300">
                              <Tag className="w-3 h-3 text-accent-500 dark:text-accent-400" />
                              {formatPrice(item.price)}
                            </span>
                          </div>
                        </div>
                        <div className="text-left">
                          <p className="font-black text-sm text-primary-700 dark:text-primary-400">{formatPrice(item.total || item.price * item.quantity)}</p>
                        </div>
                      </div>
                    </div>
                  ))}
                ...
                {/* Summary */}
                <div className="p-3 bg-gradient-to-l from-primary-50/50 to-white dark:from-primary-900/10 dark:to-slate-800 border-t border-gray-100 dark:border-slate-700">
                  <div className="space-y-2">
                    ...
                    {order.discount > 0 && (
                      <div className="flex items-center justify-between text-success-600 dark:text-success-400 text-xs">
                        <span className="flex items-center gap-1.5"><Sparkles className="w-3.5 h-3.5" />تخفیف</span>
                        <span className="font-bold">{formatPrice(order.discount)}</span>
                      </div>
                    )}
                    <div className="flex items-center justify-between text-base font-black pt-2 border-t border-primary-200 dark:border-primary-800">
            ...
            <div className="space-y-3">
              ...
              <div className="bg-white dark:bg-slate-800 rounded-xl border border-gray-100 dark:border-slate-700 shadow-sm p-3">
                <div className="flex items-center gap-1.5 mb-2">
                  ...
                  <h3 className="font-black text-gray-900 dark:text-white text-sm">اطلاعات مشتری</h3>
                </div>
                <div className="space-y-1.5">
                  <div className="flex items-center gap-2 p-2 bg-gray-50 dark:bg-slate-900/50 rounded-lg">
                    ...
                    <div className="flex-1">
                      ...
                      <p className="font-black text-gray-900 dark:text-white text-xs">{order.customer_name || order.user?.name || 'مشتری'}</p>
                    </div>
                  </div>
                  {order.user?.phone && (
                    <div className="flex items-center gap-2 p-2 bg-gray-50 dark:bg-slate-900/50 rounded-lg">
                      ...
                      <div className="flex-1">
                        <p className="text-[10px] text-gray-500 dark:text-gray-400">شماره تماس</p>
                        <p className="font-black text-gray-900 dark:text-white text-xs" dir="ltr">{order.user.phone}</p>
                      </div>
                    </div>
                  )}
                </div>
              ...
              {shippingAddress && (
                <div className="bg-white dark:bg-slate-800 rounded-xl border border-gray-100 dark:border-slate-700 shadow-sm p-3">
                  <div className="flex items-center gap-1.5 mb-2">
                    ...
                    <h3 className="font-black text-gray-900 dark:text-white text-sm">آدرس تحویل</h3>
                  </div>
                  <div className="space-y-1.5">
                    {(shippingAddress.province || shippingAddress.city) && (
                      <div className="flex items-center gap-1.5 p-2 bg-gray-50 dark:bg-slate-900/50 rounded-lg">
                        <div className="flex-1">
                          <p className="text-[10px] text-gray-500 dark:text-gray-400">استان / شهر</p>
                    ...
                    {shippingAddress.address && (
                      <div className="p-2 bg-gray-50 dark:bg-slate-900/50 rounded-lg">
                        <div className="flex items-center gap-1.5 mb-0.5">
                          ...
                          <p className="text-[10px] text-gray-500 dark:text-gray-400">آدرس کامل</p>
                        </div>
                        <p className="text-gray-900 dark:text-white text-xs leading-relaxed font-medium pr-4">
                          {shippingAddress.address}
                        </p>
                      </div>
                    )}
                    {shippingAddress.postal_code && (
                      <div className="flex items-center gap-1.5 p-2 bg-gray-50 dark:bg-slate-900/50 rounded-lg">
                        <div className="flex-1">
                          <p className="text-[10px] text-gray-500 dark:text-gray-400">کد پستی</p>
                          <p className="font-mono font-black text-gray-900 dark:text-white text-xs" dir="ltr">{shippingAddress.postal_code}</p>
                        </div>
                        <button
                          onClick={() => handleCopy(shippingAddress.postal_code!, 'کد پستی')}
                          className="p-1.5 bg-primary-100 dark:bg-primary-900/30 hover:bg-primary-200 dark:hover:bg-primary-900/50 rounded transition-colors"
                        >
                          <Copy className="w-3 h-3 text-primary-600 dark:text-primary-400" />
                        </button>
                      </div>
                    )}
                  </div>
              ...
              {order.notes && (
                <div className="bg-gradient-to-br from-warning-50 to-accent-50 dark:from-warning-900/20 dark:to-accent-900/20 border-2 border-warning-200 dark:border-warning-800/40 rounded-xl p-3">
                  <div className="flex items-center gap-1.5 mb-1.5">
                    ...
                    <h3 className="font-black text-warning-900 dark:text-warning-300 text-sm">یادداشت مشتری</h3>
                  </div>
                  <p className="text-warning-800 dark:text-warning-200 text-xs leading-relaxed bg-white/50 dark:bg-black/20 rounded-lg p-2">
                    {order.notes}
                  </p>
                </div>
              )}
      ...
      >
        <div className="text-center mb-4">
          ...
          <p className="text-gray-600 dark:text-gray-400 mt-3 text-sm">وضعیت جدید سفارش {order.order_number}</p>
        </div>
          ...
          {availableStatuses.map((status) => {
            ...
            return (
              <button
                ...
                className={cn(
                  ...
                  newStatus === status.value
                    ? 'border-primary-500 bg-gradient-to-l from-primary-50 to-white dark:from-primary-900/20 dark:to-slate-800 shadow-sm'
              ...
              >
                ...
                <span className="font-bold text-gray-900 dark:text-white text-sm flex-1">{status.label}</span>
                {newStatus === status.value && <CheckCircle className="w-4 h-4 text-primary-600 dark:text-primary-400" />}
              </button>
            );
          })}
        </div>
